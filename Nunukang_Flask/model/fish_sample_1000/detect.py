...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            # save_path = str(save_dir / 'images' / p.stem) + f'_{frame}' + '.jpg'   # im.jpg 
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if 'cigarette pack' in label:
                            ...
                            cp_width = (int(xyxy[2]) - int(xyxy[0]))  # width
                            if type(cp_width) == 'tuple':
                                cp_width = cp_width[0]
                            cp_height = (int(xyxy[3]) - int(xyxy[1]))  # height
                            if type(cp_height) == 'tuple':
                                cp_height = cp_height[0]

                            annotator.box_label(xyxy, label=None, color=colors(c, True))  # no label only draw bbox


                            if cp_width < cp_height:  
                        ...
                        if 'Red seabream' in label:  
                            ...
                            rs_height = (int(xyxy[3]) - int(xyxy[1]))
                        ...
                        if 'Black porgy' in label:  
                            annotator.box_label(xyxy, label=None, color=colors(c, True))
                            fish_name = names[c]
                            fish_name_number = c
                            bp_width = (int(xyxy[2]) - int(xyxy[0])) 
                            if type(bp_width) == 'tuple':
                                bp_width = bp_width[0]

                            bp_height = (int(xyxy[3]) - int(xyxy[1]))
                        ...
                        if 'Olive flounder' in label:
                            annotator.box_label(xyxy, label=None, color=colors(c, True))
                            fish_name = names[c]
                            fish_name_number = c
                            of_width = (int(xyxy[2]) - int(xyxy[0])) 
                            if type(of_width) == 'tuple':
                                of_width = of_width[0]

                            of_height = (int(xyxy[3]) - int(xyxy[1]))
                            if type(bp_height) == 'tuple':
                        ...
                        if 'Korea rockfish' in label:  
                            annotator.box_label(xyxy, label=None, color=colors(c, True))
                            fish_name = names[c]
                            fish_name_number = c
                            kr_width = (int(xyxy[2]) - int(xyxy[0]))
                            if type(kr_width) == 'tuple':
                                kr_width = kr_width[0]

                            kr_height = (int(xyxy[3]) - int(xyxy[1]))
                        ...
                        if 'Rock bream' in label:  
                            annotator.box_label(xyxy, label=None, color=colors(c, True))
                            fish_name = names[c]
                            fish_name_number = c
                            rb_width = (int(xyxy[2]) - int(xyxy[0]))
                            if type(rb_width) == 'tuple':
                                rb_width = rb_width[0]

                            rb_height = (int(xyxy[3]) - int(xyxy[1]))
